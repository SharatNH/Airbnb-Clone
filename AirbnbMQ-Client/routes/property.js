        ...
        geocoder.geocode(req.param("line1") +","+ req.param("city")+","+ req.param("state")+","+ req.param("country"),function ( err, data ) {
            ...
            fs.writeFile("./AirbnbMQ-Client/public/images/"+req.files[0].originalname, req.files[0].buffer, function(err) {
                ...
                fs.writeFile("./AirbnbMQ-Client/public/images/"+req.files[1].originalname, req.files[1].buffer, function(err) {
                    ...
                    userlogs={"page":"CreateListingPage","user_id":req.session.userid,"activity":"CreateListingPage","timestamp":Date.now()};
                    var property = {"propertyDetails":propertyDetails,"propertyMedia":propertyMedia,userlogs:userlogs};
...
function updateProperty(req,res){
    ...
    try {
        ...
        if(req.files[0] != undefined || req.files[1] != undefined)
        {
            ...
            fs.writeFile("./AirbnbMQ-Client/public/images/"+req.files[0].originalname, req.files[0].buffer, function(err) {
                ...
                fs.writeFile("./AirbnbMQ-Client/public/images/"+req.files[1].originalname, req.files[1].buffer, function(err) {
                    ...
                    userlogs={"page":"UpdateListingPage","user_id":req.session.userid,"activity":"UpdateListingPage","timestamp":Date.now()};
                    var updatedProperty = {"property_id":req.param("property_id"),"updatePropertyDetails":updatePropertyDetails,"updatePropertyMedia":updatePropertyMedia,userlogs:userlogs};
                    mq_client.make_request('updateProperty',updatedProperty, function(err,results) {
                        ...
                        else if(results.statusCode == 200)
                        {
                            ...
                            if(req.param("is_bidding") != 0)
                            {
                                ...
                                auction.terminateAuction({"property_id":results.property.property_id},null);
        ...
        {
            userlogs={"page":"UpdateListingPage","user_id":req.session.userid,"activity":"Update ListingPage","timestamp":Date.now()};
            var updatedProperty = {"property_id":req.param("property_id"),"updatePropertyDetails":updatePropertyDetails,"updatePropertyMedia":updatePropertyMedia,userlogs:userlogs};
            mq_client.make_request('updateProperty',updatedProperty, function(err,results) {
                if(err)
                {
                ...
                else if(results.statusCode == 200)
                {
                    ...
                    if(req.param("is_bidding") != 0)
                    {
                        ...
                        auction.terminateAuction({"property_id":req.param("property_id")},null);
...
exports.editlistingpage=function(req,res)
{
    ...
    try {
        ...
        userlogs={"page":"UpdateListingPage","user_id":req.session.userid,"activity":"updating listing profile","timestamp":Date.now()};
        mq_client.make_request('getPropertyDetails',{"property_id":property_id,userlogs:userlogs}, function(err,results) {
...
function removeProperty(req,res){
    ...
    try {
        ...
        userlogs={"page":"DashboardPage","user_id":req.session.userid,"activity":"deletion of the property","timestamp":Date.now()};
        mq_client.make_request('removeProperty',{"property_id":req.param("property_id"),"activation":0,userlogs:userlogs}, function(err,results) {
            console.log(results);
            if(err)
            {
                console.err("An error Occured.");
