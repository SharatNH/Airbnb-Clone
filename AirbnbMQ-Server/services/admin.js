...
exports.getPropRatings=function (msg,callback) {
    ...
    reviewByUser=mongo.collection('reviewByUser');
    res={};

    console.log("Inside Property Ratings with pid "+JSON.stringify(msg));

    reviewByUser.aggregate([{$match:{"property_id":msg.pid}},{"$group":{_id:"$property_id",count:{$sum:1},avgQuantity: { $avg: "$rating" }}}]).toArray(function (err,answer) {
        if(!err) { //Exception Handled
            res.code=200;
            res.value=answer;
            console.log("Ratings for Property Details as :"+JSON.stringify(res.value));
            callback(null,res);
        }
        else{
            console.log("Error from MongoDB for HOST analysis as :"+err);
...
exports.getLessSeen=function(msg,callback){
    ...
    userlogs.aggregate([{"$group":{_id:"$page",count:{$sum:1}}},{$sort:{count:1}},{$limit:3}]).toArray(function (err,answer) {
        if(!err) { //Exception Handled
...
exports.topProperties=function(msg,callback){
    ...
    connect.exec(query,[],function (rows) {
        if(rows!=undefined){
            ...
            mongoprop=mongo.collection('property');
            console.log("rows in mysql for properties contain :"+JSON.stringify(rows[0].property_id));
            imgs=[];
            for(i in rows) {
                mongoprop.findOne({"property_id": Number(rows[i].property_id)}, function (err, results) {
                    console.log("Properties from mongodb :"+results);

                    if(results!=null)
                        imgs.push(results.image);
...
exports.register=function (msg,callback) {
    ...
    connect.exec(query,params,function (rows) {
        if(rows!=undefined){
            ...
            connect.exec(query,params,function (err,rows) {
                console.log("Admin reg rows :"+rows);
                if(rows==undefined){
                    res.code=200;
                    res.value={"result":"success"}
                    callback(null,res);
